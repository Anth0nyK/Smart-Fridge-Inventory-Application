...
QString order2Model::theorderID() const
{
    return m_orderID;
}
...
void order2Model::setorderID(const QString &thetopic)
{
...
Q_INVOKABLE void order2Model::updateorderModel(){
    ...
    if (db.open()) {
        ...
        query.prepare("SELECT orders.orderID, orders.supplierName, orders.supplierEmail, orders.dateTime, orders.completed FROM orders ORDER BY orders.dateTime DESC");
...
QVariant order2Model::data(const QModelIndex &item, int role) const
{
    QVariant value;
    if (item.isValid()) {
        if (role < Qt::UserRole) {
...
QHash<int, QByteArray> order2Model::roleNames() const
{
...
order2Model::order2Model(QObject *parent):QSqlQueryModel(parent)
{
