...
QString activitylogmodel::fridgeID() const
{
    return m_fridgeID;
}
...
void activitylogmodel::setfridgeID(const QString &thetopic)
{
...
Q_INVOKABLE void activitylogmodel::updateactivitylogModel(){
    ...
    if (db.open()) {
        ...
        query.prepare("SELECT activitylog.action, activitylog.by, activitylog.dateTime FROM activitylog WHERE activitylog.fridgeID = ? ORDER BY activitylog.dateTime DESC");
...
QVariant activitylogmodel::data(const QModelIndex &item, int role) const
{
    QVariant value;
    if (item.isValid()) {
        if (role < Qt::UserRole) {
...
activitylogmodel::activitylogmodel(QObject *parent):QSqlQueryModel(parent)
{
