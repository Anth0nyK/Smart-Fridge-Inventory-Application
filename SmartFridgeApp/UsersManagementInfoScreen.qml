...
Page {
    ...
    function updateItemInfo(){
        if(nameField.text != ""){
            ...
            let suceed = 0
            suceed = theUsersManagementHelper.updateInfo(image.source, nameField.text, emailField.text, phoneField.text, thatUserID)
            if(suceed == 1){
                message.text = "Updated the user info"
                //image.source = theUsersManagementHelper.getPic(thatUserID)

                let action = "User info of user: " + theProfileHelper.getUsername(thatUserID) + " was updated"
                theActivityLogHelper.createLog(action, theProfileHelper.getUsername(currentUserID), currentfridgeID)
    ...
    function updatePW(){
        if(newpwField.text == newpw2Field.text){
            let succeed = 0
            succeed = theUsersManagementHelper.updatePW(oldpwField.text, newpwField.text, thatUserID)
            if(succeed == 1){
    ...
    Rectangle {
        ...
        Image {
            ...
            width: 166
            height: 166
            source: theUsersManagementHelper.getPic(thatUserID)
        ...
        Rectangle {
            ...
            TextInput {
                ...
                width: 146
                height: 20
                text: qsTr(theProfileHelper.getUsername(thatUserID))
                font.pixelSize: 12
            }
        }
        ...
        Rectangle {
            ...
            TextInput {
                ...
                width: 187
                height: 20
                text: qsTr(theUsersManagementHelper.getTel(thatUserID))
                font.pixelSize: 12
            }
        }
...
*/
    ...
    Rectangle {
        ...
        TextInput {
            ...
            width: 187
            height: 20
            text: qsTr(theUsersManagementHelper.getEmail(thatUserID))
            font.pixelSize: 12
        }
    }
...
/*##^##
Designer {
    D{i:0;formeditorZoom:0.9}D{i:1}D{i:2}D{i:3}D{i:4}D{i:6}D{i:7}D{i:8}D{i:9}D{i:10}D{i:11}
D{i:13}D{i:14}D{i:15}D{i:17}D{i:16}D{i:19}D{i:18}D{i:20}D{i:23}D{i:22}D{i:24}D{i:25}
D{i:26}D{i:27}D{i:28}D{i:30}D{i:32}D{i:5}D{i:35}D{i:34}D{i:37}D{i:36}D{i:39}D{i:38}
D{i:41}D{i:40}
}
##^##*/
